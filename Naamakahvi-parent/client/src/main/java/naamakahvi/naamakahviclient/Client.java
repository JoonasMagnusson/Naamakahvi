...
/**
 * A client class that passes data from user interface to server and vice versa.
 * Communication between client and server is done using HTTP. The client
 * expects the server response data to be in JSON format.
 * 
...
public class Client {
	 ...
	 */
	public static List<String> listStations(String host, int port)
			throws ClientException {
		return jsonArrayToStringList(new Client(host, port, null)
				.doGet("/list_stations/").get("stations").getAsJsonArray());
	}

	/**
	 * Creates a new Client object that uses the specified host name and port
	 * number and is located at the given station.
	 * 
	 * @param host
	 *            server host name
	 * @param port
	...
	private void checkEmptyNames(String username, String givenName,
	...
	private void checkStatusCode(final HttpResponse resp)
			throws GeneralClientException {
		final int status = resp.getStatusLine().getStatusCode();

		if (status != 200) {
			throw new GeneralClientException(
					"Server returned HTTP-status code " + status);
		}
	}

	private void checkResponseStatus(JsonObject obj)
			throws GeneralClientException {
	...
	private void checkOddParameters(String[] params)
	...
	private JsonObject responseToJson(HttpResponse response)
			throws IOException, GeneralClientException {
	...
	private JsonObject doPost(String path, String... params)
			throws ClientException {
		try {
			...
			throw new GeneralClientException("non-ClientException of class "
					+ e.getClass() + " caught: " + e.getMessage());
		}
	}

	private JsonObject doGet(String path, String... params)
			throws ClientException {
		try {
			checkOddParameters(params);
	...
	public String[] listUsernames() throws ClientException {
		return jsonArrayToStringArray(doGet("/list_usernames/", "station_name",
				this.station).get("usernames").getAsJsonArray());
	}
	...
	public void registerUser(String username, String givenName,
		...
		try {
			JsonObject obj = doPost("/register/", "username", username,
					"given", givenName, "family", familyName);
		} catch (Exception e) {
			throw new RegistrationException(e.getClass().toString() + ": "
					+ e.getMessage());
		}
	}
	...
	public void addImage(String username, byte[] imagedata)
		...
		try {
			...
			entity.addPart("file", new ByteArrayBody(imagedata, "snapshot.jpg",
					"image/jpeg"));
	...
	public IUser getUser(String username) throws AuthenticationException {
		try {
			JsonObject obj = doPost("/get_user/", "username", username,
			...
			List<SaldoItem> balance = jsonToSaldoList(data.get("balance")
					.getAsJsonArray());
	...
	private List<IProduct> jsonToProductList(JsonArray ar, boolean buyable) {
		...
		for (JsonElement e : ar) {
			...
			ans.add(new Product(productId, productName, productPrice, buyable,
					groupId, sizeId));
	...
	public List<IProduct> listBuyableProducts() throws ClientException {
		JsonObject obj = doGet("/list_buyable_products/", "station_name",
				this.station);
	 ...
	 *            bought product
	 * @param amount
	 *            amount of bought product
	 */
	public void buyProduct(IUser user, IProduct product, int amount)
			throws ClientException {
		doPost("/buy_product/", "product_id",
				Integer.toString(product.getId()), "amount", "" + amount,
				"station_name", this.station, "username", user.getUserName());
	...
	public List<IProduct> listRawProducts() throws ClientException {
		return jsonToProductList(
				doGet("/list_raw_products/", "station_name", this.station).get(
						"raw_products").getAsJsonArray(), false);
	}
	 ...
	 * @param product
	 *            the product to be brought
	 * @param amount
	 *            the amount of the brought product
	 */
	public void bringProduct(IUser user, IProduct product, int amount)
			throws ClientException {
		doPost("/bring_product/", "product_id",
				Integer.toString(product.getId()), "station_name",
				this.station, "amount", "" + amount, "username",
				user.getUserName(), "size_id",
				Integer.toString(product.getSizeId()));
	}
	...
	public String identifyImage(byte[] imagedata) throws ClientException {
		try {
			...
			JsonElement username = jsonResponse.getAsJsonObject().get(
					"username");

			return (username.isJsonNull())?null:username.getAsString();
	...
	public List<String> listProductGroups() throws ClientException {
		return jsonArrayToStringList(doGet("/list_product_groups/").get(
				"product_groups").getAsJsonArray());
	}
}
